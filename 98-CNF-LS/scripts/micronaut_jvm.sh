...
times_file="sorted_startup_times_micronaut_jvm.txt"

# Wiederhole den Vorgang
while (( read_success < 100 ))
do
  container_id=$(sudo docker create -v /home/debian/tmp/micronaut-jvm/import:/import -v /home/debian/tmp/micronaut-jvm/done:/done cr.gitlab.fhnw.ch/ip5-24vt/fileimporter/micronaut_jvm:micronaut-native-image-simplified)
...
{
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Total startup time for $read_success iterations: $total_startup_time ms using micronaut_jvm"
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Average startup time for $read_success iterations: $average_startup_time ms using micronaut_jvm"
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Median startup time for $read_success iterations: $median_startup_time ms using micronaut_jvm"
} >> "$log_file"