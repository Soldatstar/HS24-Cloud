...
times_file="sorted_startup_times_quarkus_native.txt"

# Wiederhole den Vorgang
while (( read_success < 100 ))
do
  container_id=$(sudo docker create -e API_URL=http://nginx:80 -e FILE_IMPORTER_ENABLED=true -e FILE_IMPORTER_LOCATION_TYPE=URI -e FILE_IMPORTER_IMPORT_LOCATION=/import -e FILE_IMPORTER_DONE_LOCATION=/done -e FILE_IMPORTER_WILDCARD=".+\.([Pp][Dd][Ff])\s*$" -e FILE_IMPORTER_CONFIG_NAME=testURIImporterConfig -e FILE_IMPORTER_BATCH_SIZE=2000 -e FILE_IMPORTER_INTERVAL_SEC=30 -e FILE_IMPORTER_SLEEP_BEFORE_IMPORT=2 -e FILE_IMPORTER_SLEEP_TIME=10 -e FILE_IMPORTER_THREAD_COUNT=8 -e API_USERNAME=FHNW_SERVICE_MANAGER -e API_PASSWORD=FezNNtMuauSxHGocPSF7 -v /home/debian/tmp/quarkus-native/import:/import -v /home/debian/tmp/quarkus-native/done:/done cr.gitlab.fhnw.ch/ip5-24vt/fileimporter/quarkus_native_micro:quarkus-native-image-simplified)
  ...
  while true; do
    ...
    startup_line_log=$(echo "$container_log" | grep -oP '.*started in \d+(\.\d+)?s.*')
    startup_line=$(echo "$container_log" | grep -oP 'started in \d+(\.\d+)?s.')
...
{
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Total startup time for $read_success iterations: $total_startup_time ms using quarkus_native"
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Average startup time for $read_success iterations: $average_startup_time ms using quarkus_native"
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Median startup time for $read_success iterations: $median_startup_time ms using quarkus_native"
} >> "$log_file"
