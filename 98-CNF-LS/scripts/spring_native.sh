...
times_file="sorted_startup_times_spring_native.txt"

# Wiederhole den Vorgang
while (( read_success < 100 ))
do
  container_id=$(sudo docker create -v /home/debian/tmp/spring-native/import:/import -v /home/debian/tmp/spring-native/done:/done cr.gitlab.fhnw.ch/ip5-24vt/fileimporter/spring_native:spring-native-image-simplified)
...
{
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Total startup time for $read_success iterations: $total_startup_time ms using spring_native"
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Average startup time for $read_success iterations: $average_startup_time ms using spring_native"
  echo "$(date +'%Y-%m-%d %H:%M:%S') [INFO]: Median startup time for $read_success iterations: $median_startup_time ms using spring_native"
} >> "$log_file"
